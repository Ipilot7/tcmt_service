                ...
                if ($receiveTask.IsCompleted) {
                    try {
                        ...
                        if ($data.command) {
                            ...
                            if ($cmd -eq "shutdown") {
                                Write-Host "System is shutting down..." -ForegroundColor Red
                                Stop-Computer -Force
                            }
                            elseif ($cmd -eq "reset_network") {
                                ...
                                $confirm = @{ action = "log"; data = @{ message = "Executing network reset on device $SerialNumber" } }
                                $confirmJson = $confirm | ConvertTo-Json -Compress
                                $confirmBytes = [System.Text.Encoding]::UTF8.GetBytes($confirmJson)
                                $ws.SendAsync((New-Object System.ArraySegment[byte] -ArgumentList @(,$confirmBytes)), [System.Net.WebSockets.WebSocketMessageType]::Text, $true, $cts.Token).Wait()
